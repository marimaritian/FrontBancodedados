...
const EstoqueTemplate = () => {
    ...
    async function getItems() {
        const response = await axios.get('http://localhost:3300/allitems');
        ...
        <S.Container>
            ...
            <S.Roll>
                ...
                <S.Title fontsize="35px">
                    {' '}
                    Exibindo resultados para: Todos os items
                </S.Title>
                <S.Wrapper>
                    <S.Wrapper2>
                        <S.Time>Id item</S.Time>
                        <S.Time>Id pedido</S.Time>
                        <S.Time>Id produto</S.Time>
                        <S.Time>quantidade</S.Time>
                    </S.Wrapper2>
                    {item &&
                        item.map((item: IItem) => (
                            <S.WrapperTable>
                                <S.Time2>{item.id_item_pedido}</S.Time2>
                                <S.Time2>{item.id_pedido}</S.Time2>
                                <S.Time2>{item.id_produto}</S.Time2>
                                <S.Time2>{item.quantidade}</S.Time2>
                            </S.WrapperTable>
                        ))}
                </S.Wrapper>
            </S.Roll>
